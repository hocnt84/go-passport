...
func NewOauthAccessTokenStore(db *gorm.DB, tx string) *OauthAccessTokenStore {
	return &OauthAccessTokenStore{db: db, tx: tx}
}
...
func (t *OauthAccessTokenStore) RemoveByAccessTokenId(ctx context.Context, accessTokenId string) error {
	...
	return tx.Where(&models.OauthAccessToken{
		ID: accessTokenId,
